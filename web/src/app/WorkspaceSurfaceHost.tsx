...
export const WorkspaceSurfaceHost: React.FC<{
  ...
  const open = (kind: string) => openSurface(projectSurface(project.id, kind as Parameters<typeof projectSurface>[1]));
  ...
  if(surface.type==='terminal') return surface.data?.agentId ? <div className="nx-agent-terminal-surface"><AgentTerminal agentId={surface.data.agentId}/></div> : <EmptyState title={t('surfaces.terminalUnavailable')}/>;
  if(surface.type==='resources') return <div className="nx-surface-scroll"><div className="nx-page-header"><div><span className="nx-eyebrow">{t('surfaces.resourcesEyebrow')}</span><h1>{t('surfaces.resourcesTitle')}</h1><p>{t('surfaces.resourcesIntro')}</p></div></div><div className="nx-resource-layout"><Card className="nx-resource-card"><ResourcePicker onSelect={()=>undefined}/></Card><Card className="nx-resource-card"><div className="nx-feature-heading"><Gauge size={18}/><div><strong>{t('surfaces.allocation')}</strong><small>{project.resource_policy||'BALANCED'}</small></div></div><p className="nx-muted-copy">{t('surfaces.allocationBody')}</p></Card></div></div>;
  if(surface.type==='maestro') return <div className="nx-surface-scroll"><div className="nx-page-header"><div><span className="nx-eyebrow">{t('surfaces.maestroEyebrow')}</span><h1>Maestro</h1><p>{t('surfaces.maestroIntro')}</p></div></div><MaestroPage projectId={project.id}/></div>;
  if(surface.type==='missions') return <div className="nx-surface-scroll"><div className="nx-page-header"><div><span className="nx-eyebrow">{t('surfaces.missionsEyebrow')}</span><h1>{t('surfaces.missionsTitle')}</h1><p>{t('surfaces.missionsIntro')}</p></div></div><MissionsPage projectId={project.id}/></div>;
  ...
  return <div className="nx-surface-center"><EmptyState icon={<Network size={22}/>} title={t('surfaces.unavailable')} hint={t('surfaces.unknown',{type:surface.type})} action={<Button onClick={()=>open('overview')}>{t('surfaces.openOverview')}</Button>}/></div>;
};
