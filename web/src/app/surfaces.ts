...
export function projectSurface(projectId: string, kind: ProjectSurfaceKind): WorkspaceSurface {
  const key = kind === 'legacy-runtimes' ? 'runtimes' : kind === 'legacy-providers' ? 'providers' : kind === 'legacy-events' ? 'events' : kind;
  return { id: `project:${projectId}:${kind}`, type: kind, title: labels[kind], titleKey: `nav.${key}`, closable: kind !== 'overview', data: { projectId } };
}
...
export function agentConfigSurface(agentId: string, agentName: string): WorkspaceSurface {
  return { id: `agent:${agentId}:config`, type: 'agent-config', title: `${agentName} · Configure`, titleKey: 'workspace.configureAgent', titleParams: { name: agentName }, data: { agentId }, closable: true };
}
