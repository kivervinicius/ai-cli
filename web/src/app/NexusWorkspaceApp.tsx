...
export const NexusWorkspaceApp: React.FC<{ popoutSurface?: WorkspaceSurface }> = ({ popoutSurface }) => {
  ...
  if(!sessionReady)return <div className="nx-app-loading"><Spinner label="Starting IAPro Nexus…"/></div>;
...
const NexusWorkspaceSession: React.FC<{ popoutSurface?: WorkspaceSurface }> = ({ popoutSurface }) => {
  ...
  if(data.loading)return <div className="nx-app-loading"><Spinner label="Loading Projects and Agents…"/></div>;
...
const WorkspaceCoordinator: React.FC<{ project: Project; setProject:(project:Project)=>void; data:ReturnType<typeof useNexusData>; popout:boolean }> = ({project,setProject,data,popout}) => {
  ...
  const commands=useMemo<NexusCommand[]>(()=>[
    {id:'overview',label:'Open Project Overview',group:'Project',keywords:['home'],run:()=>openKind('overview')},
    {id:'work',label:'Open Work',group:'Project',keywords:['prompt','intelligence','goal'],run:()=>openKind('work')},
    {id:'plan',label:'Open Plan / Missions',group:'Project',keywords:['mission','workplan','tasks'],run:()=>openKind('missions')},
    {id:'agents',label:'Open Agents',group:'Project',keywords:['fleet','workers'],run:()=>openKind('agents')},
    {id:'resources',label:'Open Resources',group:'Nexus',keywords:['quota','provider','accounts'],run:()=>openKind('resources')},
    {id:'maestro',label:'Open Maestro',group:'Nexus',keywords:['skills','process','verification'],run:()=>openKind('maestro')},
    {id:'sessions',label:'Open Sessions & Lineage',group:'Project',keywords:['resume','continuity'],run:()=>openKind('sessions')},
    {id:'settings',label:'Open Settings',group:'Nexus',keywords:['theme','accessibility'],run:()=>openKind('settings')},
    {id:'runtime',label:'Open Legacy Runtime Control',group:'Advanced',keywords:['runtime','legacy'],run:()=>openKind('legacy-runtimes')},
    {id:'providers',label:'Open Provider Capabilities',group:'Advanced',keywords:['provider'],run:()=>openKind('legacy-providers')},
    ...data.agents.flatMap((agent)=>[{id:`terminal-${agent.id}`,label:`Open ${agent.name} terminal`,group:'Agents',keywords:['terminal',agent.role],run:()=>terminal(agent)},{id:`config-${agent.id}`,label:`Configure ${agent.name}`,group:'Agents',keywords:['settings',agent.role],run:()=>config(agent)}]),
    {id:'tour',label:'Take Product Tour',group:'Help',keywords:['help','onboarding'],run:()=>setTour(true)},
  ],[project.id,data.agents]);
