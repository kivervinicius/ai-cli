...
const DemoCoordinator:React.FC=()=>{const workspace=useWorkspace();const[palette,setPalette]=useState(false);const[tour,setTour]=useState(false);const openKind=(kind:string)=>workspace.open(projectSurface(project.id,kind as any));const commands=useMemo<NexusCommand[]>(()=>[{id:'overview',label:'Open Overview',group:'Demo',run:()=>openKind('overview')},{id:'work',label:'Open Work',group:'Demo',run:()=>openKind('work')},{id:'settings',label:'Open Settings',group:'Demo',run:()=>openKind('settings')},{id:'tour',label:'Take Product Tour',group:'Demo',run:()=>setTour(true)}],[]);const render=(surface:WorkspaceSurface)=>{if(surface.type==='overview')return<ProjectOverviewSurface project={project} agents={agents} onOpenAgent={()=>openKind('agents')} onOpenWork={()=>openKind('work')} onOpenPlan={()=>openKind('missions')}/>;if(surface.type==='work')return<WorkSurface project={project} agents={agents} onDirect={()=>openKind('agents')} onPlan={()=>openKind('missions')} onMaestro={()=>openKind('maestro')}/>;if(surface.type==='settings')return<SettingsSurface onTour={()=>setTour(true)}/>;return<div className="nx-surface-scroll"><div className="nx-page-header"><div><span className="nx-eyebrow">Synthetic Demo</span><h1>{surface.title}</h1><p>This demo surface uses synthetic data only. Connect the real backend for operational actions.</p></div><Badge tone="warning">DEMO</Badge></div><Card><strong>Workspace OS preview</strong><p className="nx-muted-copy">Use tabs, split controls, drag/drop, maximize, taskbar and the command palette without touching real provider sessions.</p></Card></div>};return<><NexusShell project={project} agents={agents} rail={<div className="nx-demo-rail"><img src="./logo.png" alt="IAPro Nexus" className="nx-brand-mark nx-brand-mark-img" /><strong>DEMO</strong><small>Synthetic data</small></div>} onOpenRail={()=>undefined} onOpenSurface={openKind} onCommand={()=>setPalette(true)} onOpenWelcome={()=>setTour(true)} onOpenProjectManager={()=>undefined} onOpenMaestroControl={()=>openKind('maestro')} onSettings={()=>openKind('settings')}><WorkspaceRenderer renderSurface={render} popoutSurface={()=>undefined}/></NexusShell><CommandPalette open={palette} onClose={()=>setPalette(false)} commands={commands}/><ProductTour open={tour} onClose={()=>setTour(false)}/></>};
