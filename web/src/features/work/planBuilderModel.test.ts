...
const plan = (): WorkPlan => ({
  ...
  created_at: '', updated_at: '', phases: [{ id: 'phase-1', title: 'Phase', order: 1, packages: [
    { id: 'a', title: 'Foundation', goal: 'base', priority: 'HIGH', status: 'READY', dependencies: [], role: 'implementer', acceptance_criteria: ['schema', 'tests'] },
    { id: 'b', title: 'API', goal: 'api', priority: 'HIGH', status: 'READY', dependencies: ['a'], role: 'implementer', acceptance_criteria: ['endpoint'] },
  ] }],
});
...
describe('WorkPlan structural editing', () => {
  ...
  it('merges packages and rewrites downstream dependencies to the survivor', () => {
    const source = plan();
    source.phases[0].packages.push({ id: 'c', title: 'Review', goal: 'review', priority: 'NORMAL', status: 'READY', dependencies: ['b'], role: 'reviewer', acceptance_criteria: ['approved'] });
...
describe('Provider lock', () => {
  it('writes and clears provider/profile hard locks without destroying other requirements', () => {
    ...
    expect(JSON.parse(locked)).toMatchObject({ required_capabilities: ['headless'], estimated_tokens: 1000, provider_lock: 'claude', profile_lock: 'review' });
    expect(JSON.parse(setProviderLock(locked, '', ''))).toEqual({ required_capabilities: ['headless'], estimated_tokens: 1000 });
  });
});
...
describe('AI suggestion diff', () => {
  it('compares a generated plan without mutating the approved plan', () => {
    ...
    suggestion.phases[0].packages[1] = { ...suggestion.phases[0].packages[1], goal: 'new api goal' };
    suggestion.phases[0].packages.push({ id: 'new', title: 'Security', goal: 'audit', priority: 'HIGH', status: 'READY', dependencies: [], role: 'reviewer', acceptance_criteria: ['clean'] });
