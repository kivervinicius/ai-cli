...
export const FlowStepInspector: React.FC<{
  ...
  return <Card className="nx-flow-inspector">
    ...
    <label><span>Goal</span><Textarea rows={4} value={step.goal} onChange={(goal)=>onChange({goal})}/></label>
    <div className="nx-flow-inspector__grid">
      <Select label="Assignment" value={step.assignmentStrategy} onChange={(value)=>onChange({assignmentStrategy:value as FlowStepModel['assignmentStrategy'],agentId:value==='EXISTING'?step.agentId:undefined})} options={[{value:'EXISTING',label:'Existing Agent'},{value:'CREATE',label:'Create specialist'},{value:'AUTO',label:'Auto / Scheduler'}]}/>
      {step.assignmentStrategy==='EXISTING'?<Select label="Agent" value={step.agentId||''} onChange={(agentId)=>onChange({agentId})} placeholder="Select Agent" options={agents.map((agent)=>({value:agent.id,label:`${agent.name} · ${agent.status}`}))}/>:<label><span>Role</span><Input value={step.role} onChange={(role)=>onChange({role})}/></label>}
      <Select label="Resource policy" value={step.resourcePolicy||'BALANCED'} onChange={(resourcePolicy)=>onChange({resourcePolicy})} options={[{value:'BALANCED',label:'Balanced'},{value:'PRESERVE_QUOTA',label:'Preserve quota'},{value:'PREFER_PROVIDER',label:'Prefer provider'},{value:'MANUAL',label:'Manual restriction'}]}/>
      <label><span>Parallel group</span><Input value={step.parallelGroup||''} onChange={(parallelGroup)=>onChange({parallelGroup:parallelGroup||undefined})}/></label>
    ...
    <label><span>Verification requirements · one per line</span><Textarea rows={3} value={joined(step.verificationRequirements)} onChange={(value)=>onChange({verificationRequirements:lines(value)})}/></label>
    <div className="nx-flow-inspector__actions"><Button size="sm" onClick={()=>onAction('REFINE')}><WandSparkles size={12}/>Refine locally</Button><Button size="sm" onClick={()=>onAction('EXPAND')}><Rows3 size={12}/>Expand</Button><Button size="sm" onClick={()=>onAction('COMPARE')}><Diff size={12}/>Compare</Button><Button size="sm" onClick={()=>onAction('SPLIT')}><Scissors size={12}/>Split</Button><Button size="sm" tone="danger" onClick={()=>onAction('REMOVE')}><Trash2 size={12}/>Remove</Button></div>
  </Card>;
};
