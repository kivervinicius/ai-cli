...
export function splitPackage(
  ...
  const first: WorkPackage = {
    ...source,
    title: `${source.title} · Parte 1`,
    acceptance_criteria: firstAcceptance,
  };
  ...
  for (const candidatePhase of next.phases) {
    for (const pkg of candidatePhase.packages) {
      ...
      pkg.dependencies = unique(
        (pkg.dependencies || []).map((dep) => (dep === source.id ? second.id : dep)),
      );
    }
  }
...
export function mergePackages(
  ...
  if (survivorIndex < 0 || mergedIndex < 0)
    throw new Error('merge packages must be in the selected phase');
  const survivor = phase.packages[survivorIndex];
  const merged = phase.packages[mergedIndex];

  survivor.title =
    survivor.title === merged.title ? survivor.title : `${survivor.title} + ${merged.title}`;
  survivor.goal = unique([survivor.goal, merged.goal]).join('\n\n');
  survivor.acceptance_criteria = unique([
    ...(survivor.acceptance_criteria || []),
    ...(merged.acceptance_criteria || []),
  ]);
  survivor.shared_artifacts = unique([
    ...(survivor.shared_artifacts || []),
    ...(merged.shared_artifacts || []),
  ]);
  survivor.dependencies = unique(
    [...(survivor.dependencies || []), ...(merged.dependencies || [])].filter(
      (dep) => dep !== survivorId && dep !== mergedId,
    ),
  );
  ...
  for (const candidatePhase of next.phases) {
    for (const pkg of candidatePhase.packages) {
      pkg.dependencies = unique(
        (pkg.dependencies || [])
          .map((dep) => (dep === mergedId ? survivorId : dep))
          .filter((dep) => dep !== pkg.id),
      );
    }
  }
  return next;
...
export function setProviderLock(
  ...
  if (provider.trim()) requirements.provider_lock = provider.trim();
  else delete requirements.provider_lock;
  if (profile.trim()) requirements.profile_lock = profile.trim();
  else delete requirements.profile_lock;
  return JSON.stringify(requirements);
}

export function getProviderLock(rawRequirements: string | undefined): {
  provider: string;
  profile: string;
} {
...
export function planSuggestionDiff(current: WorkPlan | null, suggestion: WorkPlan) {
  ...
  for (const phase of current?.phases || [])
    for (const pkg of phase.packages) currentByTitle.set(pkg.title, pkg);
  const suggestedByTitle = new Map<string, WorkPackage>();
  for (const phase of suggestion.phases)
    for (const pkg of phase.packages) suggestedByTitle.set(pkg.title, pkg);
  ...
  for (const [title, pkg] of suggestedByTitle) {
    ...
    const comparable = (value: WorkPackage) =>
      JSON.stringify({
        goal: value.goal,
