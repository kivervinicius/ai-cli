...
export const AskAgentDialog: React.FC<{
  ...
  return (
    <Dialog
      open={!!agent}
      onClose={onClose}
      title={agent ? `${action.label} · ${agent.name}` : action.label}
      wide
    >
      <div className="nx-ask-agent-dialog">
        <p className="nx-muted-copy">
          Sends work to this persistent Agent identity. No new Agent is created.
        </p>
        <Textarea
          rows={7}
          value={prompt}
          onChange={setPrompt}
          placeholder="Describe what this Agent should do next…"
          autoFocus
        />
        ...
        <div className="nx-dialog-actions">
          ...
          <Button tone="brand" disabled={!prompt.trim() || busy} onClick={() => void submit()}>
            {action.startIfNeeded ? <Play size={14} /> : <MessageSquareText size={14} />}{' '}
