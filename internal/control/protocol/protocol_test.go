...
func TestSubmitPromptRequestUsesDedicatedCommandAndPayload(t *testing.T) {
	...
	if req.Command != CmdSubmitPrompt { t.Fatalf("command=%s", req.Command) }
	var payload SubmitPromptPayload
	if err := json.Unmarshal(req.Payload, &payload); err != nil { t.Fatal(err) }
	if payload.Prompt != "/ai literal prompt" { t.Fatalf("prompt=%q", payload.Prompt) }
}
